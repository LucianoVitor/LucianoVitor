🇺🇸 [English version here](./readme.en.md)

<div align="center">
<img src="./luciano-rpg-card.png" alt="Ficha de personagem" width="700"/>
</div>

<div align="center">
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExMGhpZmEyc3ZpeW1rbW1ybmxnZ203dGlwMjJkZ2ZkZWVoNzV0dWVpNCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/EZr27ZbJwmjE9PGyLN/giphy.gif" height="330" />
</div>

---

## 🧭 Sobre mim

Olá! Sou o **Luciano Vitor**, desenvolvedor **Frontend & Mobile** em formação pela **Fatec Zona Leste** (São Paulo/SP). Tenho sólida experiência prática no desenvolvimento de aplicações modernas e interfaces responsivas, unindo paixão por tecnologia e design fluido.

Com formação técnica prévia em Desenvolvimento de Sistemas pela **Etec Zona Leste**, foco em criar códigos limpos, otimizados e focados na melhor experiência do usuário.

Apaixonado por **design de interfaces**, **animações fluidas** e aquela sensação de ver um componente React ou uma tela mobile funcionando perfeitamente em qualquer dispositivo. 🎯

---

## 🏰 Experiências e Projetos

<table>
  <tr>
    <td width="50%">
      <h3>⚙️ Lead Developer & Frontend</h3>
      <strong>CicloTECH · Fatec ZL</strong><br/>
      Liderei o desenvolvimento e a arquitetura da plataforma CicloTECH, uma iniciativa voltada para a economia circular de equipamentos de TI na Zona Leste. Responsável pela padronização estrutural, interface com o usuário e gerenciamento do projeto.
      <br/><br/>
      <code>React</code> <code>Vite</code> <code>Tailwind CSS</code> <code>Workspace</code>
    </td>
    <td width="50%">
      <h3>📚 Estagiário de Tecnologia</h3>
      <strong>SEDUC / PROATI</strong><br/>
      Atuação profissional na Secretaria da Educação do Estado de São Paulo, colaborando com processos técnicos e desenvolvimento para o setor público de educação.
      <br/><br/>
      <code>Sistemas</code> <code>Resolução de Problemas</code> <code>Processos Digitais</code>
    </td>
  </tr>
</table>

---

## ⚔️ Arsenal Tecnológico

**Frontend & Mobile**
<br/>
<img src="https://skillicons.dev/icons?i=react,vite,tailwind,js,html,css" />

**Backend & Banco de Dados**
<br/>
<img src="https://skillicons.dev/icons?i=java,mysql,sqlite" />

**Ferramentas & Ambiente**
<br/>
<img src="https://skillicons.dev/icons?i=git,github,vscode,bash,linux,figma,vercel" />

---

## 🌟 Diferenciais
🧠  Olho clínico para Design →  Foco em interfaces limpas, modernas e de alto padrão (Tailwind & Assets organizados)
🤝  Trabalho Colaborativo   →  Experiência liderando times acadêmicos na Fatec e integrando soluções reais
🎯  Organização de Projeto   →  Eliminação de gargalos, refatoração de estrutura de pastas e consistência de código
📖  Evolução Constante      →  De background em Java migrado para especialização sólida em Frontend & Mobile
🗣️  Visão de Negócio        →  Alinhando código com impacto sustentável e social (como o CicloTECH)

---

## 📊 Stats do GitHub

<div align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=LucianoVitor&hide_title=false&hide_rank=false&show_icons=true&include_all_commits=true&count_private=true&disable_animations=false&theme=shades-of-purple&locale=en&hide_border=false&order=1" height="150" alt="stats graph" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs?username=LucianoVitor&locale=en&hide_title=false&layout=compact&card_width=320&langs_count=5&theme=shades-of-purple&hide_border=false&order=2" height="160" alt="languages graph" />
</div>

<br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=LucianoVitor&radius=16&theme=elegant&area=true&order=5&hide_border=false&hide_title=true" height="300" alt="activity-graph graph" />

---

## 🟡 Mapa de Contribuições

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/LucianoVitor/LucianoVitor/snk-generated/github-contribution-grid-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/LucianoVitor/LucianoVitor/snk-generated/github-contribution-grid-snake.svg">
  <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/LucianoVitor/LucianoVitor/snk-generated/github-contribution-grid-snake.svg">
</picture>

---

## 💼 Projetos em Destaque

<table>
  <tr>
    <td align="left">
      <a href="https://github.com/LucianoVitor/CicloTECH" target="_blank">
        <img width="400" alt="CicloTECH" src="https://github-readme-stats.vercel.app/api/pin/?username=LucianoVitor&repo=CicloTECH&theme=shades-of-purple&hide_border=false" />
      </a>
    </td>
    <td align="left">
      <a href="https://github.com/LucianoVitor/Album_copa_mobile" target="_blank">
        <img width="400" alt="Album da Copa Mobile" src="https://github-readme-stats.vercel.app/api/pin/?username=LucianoVitor&repo=Album_copa_mobile&theme=shades-of-purple&hide_border=false" />
      </a>
    </td>
  </tr>
</table>

---

## 📫 Fala comigo!

> 💡 *Sempre aberto a desafios no ecossistema Frontend e Mobile, projetos inovadores e conexões na área de desenvolvimento!*

<div align="left">
  <a href="https://www.linkedin.com/in/LucianoVitor/" target="_blank">
    <img src="https://img.shields.io/static/v1?message=LinkedIn&logo=linkedin&label=&color=0077B5&logoColor=white&labelColor=&style=for-the-badge" height="30" alt="linkedin logo" />
  </a>
  <a href="mailto:seu.email.luciano@gmail.com" target="_blank">
    <img src="https://img.shields.io/static/v1?message=Gmail&logo=gmail&label=&color=D14836&logoColor=white&labelColor=&style=for-the-badge" height="30" alt="gmail logo" />
  </a>
</div>

<br/>

<div align="center">
  <sub>⚔️ Obrigado por visitar meu perfil — cada commit é um passo na jornada! ⭐</sub>
</div>
